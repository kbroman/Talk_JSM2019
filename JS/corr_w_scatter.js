...
d3.json("Data/corr_w_scatter.json", function(data) {
  var cells, colorScale, colors, corXscale, corYscale, corZscale, corr, corrplot, drawScatter, extraPad, firsttime, h, i, innerPad, j, nGroup, nind, nvar, pad, scatterplot, svg, totalh, totalw, w;
  h = 450;
  w = h;
  pad = {
    left: 70,
  ...
  cells = corrplot.selectAll("empty").data(corr).enter().append("rect").attr("class", "cell").attr("x", function(d) {
    ...
    return d3.select("font.heatmap_hide").style("opacity", 1).attr("class", "lowlight");
  });
