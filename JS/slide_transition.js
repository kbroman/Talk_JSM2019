...
var cistrans_index, corr_w_scatter_index, current_slide, d3_index, density_index, get_slide_index, lod_and_effect_index, lod_over_time_index, manyboxplots_index, permutation_index, slide_transition;
...
slide_transition = function(slidenumber) {
  ...
  if (slidenumber === density_index + 1 || slidenumber === density_index - 1) {
    stop_density();
  }
  ...
  if (slidenumber === manyboxplots_index - 1 || slidenumber === manyboxplots_index + 1) {
    stop_manyboxplots();
  }
  ...
  if (slidenumber === cistrans_index - 1 || slidenumber === cistrans_index + 1) {
    stop_cistrans();
  }
  if (slidenumber === d3_index) {
    console.log("animate d3 slide");
    d3.select("p#dotenter").transition().style("opacity", 1).delay(5000).duration(5000).ease("linear");
    collision();
  }
