  ...
  drawRandom = function(data, column) {
    ...
    for (j = 0, len = ref.length; j < len; j++) {
      ...
      for (k = 0, len1 = ref1.length; k < len1; k++) {
        lod = ref1[k];
        if (maxLod < lod) { // overall maximum LOD
          maxLod = lod;
        }
      }
    ...
    // vertical scales
    lodyScale = d3.scale.linear().domain([
      0,
      5.5 // hard-coded maximum so this and the non-randomized version have same scale
...
stop_permutation = function() {
  return d3.selectAll("div#lod_onetime_random_fig svg").remove();
};
